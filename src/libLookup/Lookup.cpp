...
void Lookup::InitSync() {
  ...
  auto func = [this]() -> void {
    ...
    while (GetSyncType() != SyncType::NO_SYNC) {
      LOG_GENERAL(INFO, "NodeRejoin: Inside InitSync created thread");
      ...
      if (m_mediator.m_lookup->cv_setRejoinRecovery.wait_for(
        ...
        LOG_GENERAL(INFO, "NodeRejoin: Break from the loop");
        break;
    ...
    LOG_GENERAL(INFO, "###### NodeRejoin: InitSync finished thread ######");
  };
...
bool Lookup::ProcessSetTxBlockFromSeed(
  ...
  if (lowBlockNum > highBlockNum) {
    ...
    // continue while loop after timeout of 300 secs
    return false;
  }
  ...
  if (latestSynBlockNum > highBlockNum) {
    ...
    if (latestSynBlockNum < lowBlockNum) {
      ...
      LOG_GENERAL(INFO,
                  "NodeRejoin: Seems we have received all or few of txblocks "
                  "previously. "
...
bool Lookup::CommitTxBlocks(const vector<TxBlock>& txBlocks) {
  ...
  if (m_syncType != SyncType::RECOVERY_ALL_SYNC) {
    ...
    // TODO:Delete these lines later on
    // ToReproduce state hash failure and delete above code lines
    /*
...
bool Lookup::ProcessSetStateDeltasFromSeed(
  ...
  /* Reproduce Deserialize failed scenario
  m_reproduceIssue++;
...
void Lookup::RejoinNetwork() {
  ...
  if (m_syncType == SyncType::NEW_LOOKUP_SYNC) {
    if (ARCHIVAL_LOOKUP) {
      ...
      LOG_GENERAL(WARNING, "Alas !!! Unhandled rejoin scenario. SyncType="
                               << m_syncType);
    }
  }
}
