...
void Lookup::UpdateNodes(VectorOfNode& resultNodes, unsigned int nodePort,
  ...
  VectorOfNode tmp;
  if (QueryNodesFromDns(tmp, dns, nodePort)) {
  ...
  LOG_GENERAL(INFO,
              "Size of " << fallbackXmlType << ": " << resultNodes.size());
}
...
void Lookup::SendMessageToLookupNodes(const bytes& message) const {
  ...
  if (ipList.size() == 0) {
    ...
    for (const auto& node : lookups) {
      ipList.emplace_back(node.second.GetIpAddress());
    }
  }
...
void Lookup::SendMessageToRandomLookupNode(const bytes& message) {
  ...
  VectorOfNode localLookupList = GetLookupNodesWithoutMultipliers();
  localLookupList.erase(
      std::remove_if(localLookupList.begin(), localLookupList.end(),
                     [selfIp](const PairOfNode& node) {
                       return node.second.GetIpAddress() == selfIp;
                     }),
      localLookupList.end());
  ...
  if (lookupIpsFromDns.empty()) {
    ...
    if (localLookupList.empty()) {
      LOG_GENERAL(WARNING, "There is no lookup node existed yet!");
      return;
    }

    int index = RandomGenerator::GetRandomInt(localLookupList.size());
    randomPeer =
        Peer{localLookupList[index].second.GetIpAddress(), DEFAULT_SEED_PORT};

  } else {
    // Find if we have any intersection between dns list and our current list
    vector<uint128_t> choices;
    if (!localLookupList.empty()) {
      std::copy_if(
          lookupIpsFromDns.begin(), lookupIpsFromDns.end(),
          std::back_inserter(choices), [&localLookupList](const uint128_t& ip) {
            return find_if(localLookupList.begin(), localLookupList.end(),
                           [&ip](const PairOfNode& node) {
                             return node.second.GetIpAddress() == ip;
                           }) != localLookupList.end();
          });
    }
...
void Lookup::SendMessageToSeedNodes(const bytes& message) const {
  ...
  if (!ObtainIpListFromDns(seedIps, UPPER_SEED_DNS)) {
    LOG_GENERAL(WARNING, "Unable to obtain IPs from DNS "
    ...
    for (const auto& node : localSeeds) {
      seedIps.emplace_back(node.second.GetIpAddress());
    }
  }
...
void Lookup::SendMessageToRandomL2lDataProvider(const bytes& message) {
  ...
  if (!ObtainIpListFromDns(ipFromDns, L2L_DATA_PROVIDERS_DNS) ||
      ipFromDns.empty()) {
    LOG_GENERAL(WARNING, "Unable to obtain IPs from DNS "
                             << L2L_DATA_PROVIDERS_DNS
                             << ", using current list.");

    if (localL2lDataProvider.empty()) {
      LOG_GENERAL(WARNING, "No L2lDataProvider to send to");
      return;
    }

    int index = RandomGenerator::GetRandomInt(localL2lDataProvider.size());
    randomPeer = Peer{localL2lDataProvider[index].second.GetIpAddress(), port};
  } else {
    // Find if list of L2lDataProvider from dns that intersects with local list
    vector<uint128_t> intersectedList;
    std::copy_if(ipFromDns.begin(), ipFromDns.end(),
                 std::back_inserter(intersectedList),
                 [&localL2lDataProvider](const uint128_t& ip) {
                   return find_if(localL2lDataProvider.begin(),
                                  localL2lDataProvider.end(),
                                  [&ip](const PairOfNode& node) {
                                    return node.second.GetIpAddress() == ip;
                                  }) != localL2lDataProvider.end();
                 });
...
void Lookup::SendMessageToRandomSeedNode(const bytes& message) {
  ...
  if (ObtainIpListFromDns(ipsFromDns, UPPER_SEED_DNS)) {
  ...
  localSeedNodes.erase(
      std::remove_if(localSeedNodes.begin(), localSeedNodes.end(),
                     [&selfIp](const PairOfNode& node) {
                       const auto& ip = node.second.GetIpAddress();
                       return ip == selfIp ||
                              Blacklist::GetInstance().Exist(ip);
                     }),
      localSeedNodes.end());
  ...
  if (ipsFromDns.empty()) {
    ...
    if (localSeedNodes.empty()) {
      LOG_GENERAL(WARNING, "There is no seed node yet!");
      return;
    }

    // Use a random local one
    int index = RandomGenerator::GetRandomInt(localSeedNodes.size());
    randomPeer =
        Peer{localSeedNodes[index].second.GetIpAddress(), DEFAULT_SEED_PORT};
  } else {
    // Find seeds from dns that intersect in local seed records
    vector<uint128_t> filteredSeedIps;
    if (!localSeedNodes.empty()) {
      std::copy_if(ipsFromDns.begin(), ipsFromDns.end(),
                   std::back_inserter(filteredSeedIps),
                   [&localSeedNodes](const uint128_t& ip) {
                     return find_if(localSeedNodes.begin(),
                                    localSeedNodes.end(),
                                    [&ip](const PairOfNode& node) {
                                      return node.second.GetIpAddress() == ip;
                                    }) != localSeedNodes.end();
...
void Lookup::RemoveSeedNodesFromBlackList() {
  ...
  vector<uint128_t> ipList;
  if (!ObtainIpListFromDns(ipList, UPPER_SEED_DNS)) {
    LOG_GENERAL(WARNING, "Unable to obtain IPs from DNS "
                             << UPPER_SEED_DNS << ", using current list.");
    {
      shared_lock<shared_timed_mutex> lock(m_mutexSeedNodes);
