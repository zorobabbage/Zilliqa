...
//   PairOfKey employee3 = Schnorr::GenKeyPair();
  ...
  unsigned int num_sender = 100000;
...
//   employee3Addr = Account::GetAddressFromPublicKey(employee3.second);
  ...
  for (unsigned int i = 0; i < num_sender; ++i) {
    ...
    AccountStore::GetInstance().AddAccountTemp(senderAddr, {2000000000000, nonce});
  }
...
//   tests[4].message["params"][0]["value"] = "0x" + employee1Addr.hex();
  ...
  for (unsigned int i = 0; i < tests.size();) {
    ...
    std::string initStr = JSONUtils::GetInstance().convertJsontoStr(tests[0].init);
    bytes data;
    uint64_t amount = 0;
    Address recipient;
    bytes code;
  ...
  for (unsigned int i = 0; i < num_sender; ++i) {
    ...
    Transaction tx(DataConversion::Pack(CHAIN_ID, 1), 0, contrAddr, senders[i], stressAmount, PRECISION_MIN_VALUE, 20000, {}, stressData);
    TransactionReceipt tr;
